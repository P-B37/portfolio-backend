...
def set_project_slug(sender, instance: Project, **kwargs):
    """
    Signal receiver to set a unique slug
    for a Project instance before saving.

    This function generates a slug based
    on the project's title and ID,
    ensuring that each project has a unique
      and SEO-friendly identifier.

    Args:
        sender (Model): The model class
        sending the signal (Project).
